...
namespace tests.Features.Example
{
    public class ExampleControllerTests : ApiTest
    {
        ...
        [Fact]
        public void Create_FailModelValidation()
        {
            var createRequest = new CreateExampleRequest();
            ...
            action.Should().Throw<FlurlHttpException>().Which.StatusCode.Should().Be(StatusCodes.Status400BadRequest);
        }

        [Fact]
        public void Create_FailFluentValidation()
        {
            var createRequest = new CreateExampleRequest
            {
                Name = "Bad name"
